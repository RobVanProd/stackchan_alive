...
if (-not (Test-Path -LiteralPath $ApkPath)) {
  throw "Missing Android APK: $ApkPath. From the source checkout, for a lab-only build run cd companion; .\gradlew.bat `"-Pstackchan.allowLabDebugReleaseSigning=true`" :app-android:assembleRelease, then pass the generated APK path with -ApkPath. The unqualified .\gradlew.bat :app-android:assembleRelease command is intentionally rejected without upload signing."
}
