...
if ($InstallSox -and -not $beforeSox.found) {
  try {
    ...
    $installFailures += [pscustomobject]@{
      ...
      error = $_.Exception.Message
      note = "SoX is optional; Stackchan Spark Synth v2 does not require it."
    }
    if (-not $ContinueOnInstallFailure) {
      throw
    }
...
$status = [ordered]@{
  ...
  optionalPostProcessing = if ($afterSox.found) { "SoX available for external audition experiments" } else { "SoX not required; Stackchan Spark Synth v2 DSP is built into render_voice_samples.ps1" }
