...
$report = [ordered]@{
  ...
  applicationId = if ($null -ne $evidence) { [string]$evidence.applicationId } else { "" }
  versionName = if ($null -ne $evidence) { [string]$evidence.versionName } else { "" }
  versionCode = if ($null -ne $evidence) { [string]$evidence.versionCode } else { "" }
  releaseAabSha256 = if ($null -ne $evidence) { [string]$evidence.releaseAabSha256 } else { "" }
  root = [string]$Root
