...
if ([string]::IsNullOrWhiteSpace($OutputDir)) {
  $OutputDir = Join-Path $repoRoot "output/engine-probe/latest"
}

$args = @(
  (Join-Path $repoRoot "bridge/engine_probe.py"),
  "--out-dir",
  $OutputDir
)
...
if ($LASTEXITCODE -ne 0) {
  throw "Engine probe failed."
}

if (-not $Json) {
  Write-Host "Engine probe report:"
