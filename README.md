...
- Firmware-side WebSocket handshake/frame adapter and trusted-endpoint owner registry for `stackchan.bridge.v1`, with native tests for masked client frames, server text frames, PCM16 binary downlink chunks, disconnect handling, owner handoff, and endpoint forgetting.
...
- Real firmware Wi-Fi provisioning, TCP/WebSocket task hookup, endpoint persistence, and PC/mobile owner failover evidence on the CoreS3.
