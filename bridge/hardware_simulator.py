...
class VirtualStackchanHardware:
    ...
    def _process_text(self, frame: dict[str, object]) -> None:
        ...
        if frame_type == "response_start":
            ...
            self.telemetry.active_seq = _int(frame.get("seq"), self.telemetry.active_seq)
            self.telemetry.face_mode = str(frame.get("intent") or "speak")
...
def build_arg_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--scenario",
        action="append",
        choices=("reference", "lan-text", "audio-downlink", "timeout"),
        help="Scenario to run. Defaults to reference, lan-text, and audio-downlink.",
    )
...
def main() -> int:
    ...
    scenarios = args.scenario or ["reference", "lan-text", "audio-downlink"]
    if args.out_dir:
