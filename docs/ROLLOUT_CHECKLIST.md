...
Speech-mouth evidence must include `logs/speech_mouth_demo_serial.log` with streamed `speech` commands, `speech clear`, and demo completion, plus `logs/speak_all_intents_serial.log` with every packaged speech intent and `[audio_out]` handoff.
...
- [ ] `RUN_SPEAK_ALL_INTENTS.cmd` writes `logs/speak_all_intents_serial.log` with `speak <intent>` coverage for boot through safety and packaged prompt `[audio_out]` telemetry.
