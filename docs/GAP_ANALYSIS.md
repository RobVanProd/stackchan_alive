...
WebSocket adapter plus native-tested socket-writer text/binary uplink queues, but the running
...
`UserSpeaking`/`SoundDirection`/`SpeechEnded` reflex events. The adapter also exposes the
