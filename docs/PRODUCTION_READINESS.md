...
- `tools/check_voice_source_readiness.cmd -Json` audits the active production voice-source gate without mutating release artifacts; it should remain `pending-production-voice-source` until the licensed/owned source, consent/license evidence, template attestations, and target-speaker evidence are complete.
