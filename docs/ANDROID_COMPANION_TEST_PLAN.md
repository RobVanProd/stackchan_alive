...
- [ ] For firmware without a consumer Wi-Fi menu, confirm the Android setup card shows a `Robot Wi-Fi setup` command template using the current phone bridge URL, then send `wifi set ssid <network-name> pass <network-password> url <ws://phone-lan-ip:8765/bridge>` on the robot serial console, verify `[wifi]` reports `persisted=1`, `store_has_record=1`, `enabled=1`, and `ssid_set=1` without echoing the password, power-cycle the robot and verify `bridge_wifi_store_loads` increments, then send `wifi clear` and verify `store_has_record=0`.
