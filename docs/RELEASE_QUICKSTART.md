...
This serves the release ZIP, preview image, expression sheet, video, quickstart, release notes, and checksums. It downloads a local `cloudflared.exe` under `output\tools` only when `cloudflared` is not already installed.
