...
the `bridge-tests` job. It uploads `output/hardware-sim/latest/` as the
`hardware-simulation` artifact and `output/prearrival-sim/latest/` as the
`prearrival-simulation-check` artifact.
