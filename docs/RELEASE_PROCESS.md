...
The package is written under `output/release/<version>/` and includes firmware binaries, preview media, an expression QA sheet, a root `QUICKSTART.md`, readiness docs, dependency provenance, a machine-readable dependency lock, a dependency audit, copied build inputs, flash helpers, and SHA256 checksums.
...
Stage a local handoff page with direct links to the ZIP, image, expression sheet, video, GIF, release notes, and checksums:
...
Run `tools/verify_share_release.cmd -Version <version> -RequirePublicUrl` before sending the URL; it checks the handoff page plus the preview PNG, expression sheet, MP4, GIF, and ZIP over HTTP.
